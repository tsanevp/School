...
int InsertInsideLinkedList(LinkedListPtr list, int value, int pos) {
    ...
    // If the pos is greater than the last index in the list or list is empty,
    // append new node
...
int RemoveLLElem(LinkedListPtr linkedList, LinkedListNodePtr nodePtr) {
    ...
    // If: In the case the node is head
    // Else if: Checks all nodes after head until tail
    // Else if: In the case the node is tail
