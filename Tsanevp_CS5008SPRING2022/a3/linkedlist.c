...
int InsertInsideLinkedList(LinkedListPtr list, int value, int pos) {
    ...
    // Edge Case:
    // If list not empty and pos is <= index 0, make new node list head
    if (list->head != NULL && pos <= 0) {
        return InsertLinkedList(list, value);
    }

    // Edge Case:
    // If the pos is greater than the last index in the list or
    // list is empty, append new node
    if ((list->head == NULL && list->tail == NULL) ||
        (pos > (NumElementsInLinkedList(list)-1))) {
...
int RemoveLLElem(LinkedListPtr linkedList, LinkedListNodePtr nodePtr) {
    ...
    // Checks all nodes after head until tail
        ...
        temp->next = NULL;
        temp->prev = NULL;
    } else if (temp == linkedList->tail) {  // In the case the node is tail
        (temp->prev)->next = NULL;
